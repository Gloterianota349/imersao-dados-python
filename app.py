...
# Tenta carregar o CSV localmente primeiro (mais confiável em deploys locais/Cloud),
# senão tenta a URL raw correta do GitHub como fallback.
local_path = "dados-imersao-final.csv"
remote_url = "https://raw.githubusercontent.com/vqrca/dashboard_salarios_dados/main/dados-imersao-final.csv"
try:
...
st.markdown("Analise os salários na área de dados nos últimos anos, utilizando diferentes filtros.")
...
with col_graf1:
    if not df_filtrado.empty:
        try:
            ...
            st.error(f"Erro ao gerar gráfico de cargos: {e}")
    else:
...
with col_graf3:
    if not df_filtrado.empty:
        try:
            ...
            st.error(f"Erro ao gerar gráfico de tipo de trabalho: {e}")
    else:
        st.warning("Nenhum dado disponível para os filtros selecionados.")
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        grafico_paises = px.choropleth(
            ...
            title = 'Média Salarial Anual por País (Data Scientist)',
